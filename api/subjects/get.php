...
$columns = ['course_no', 'name', 'description', 'status', 'created_at'];
...
//Get total
$totalRecords = $subjects->getTotal($whereCondition, $sortBy);

//Get all tsag
$results = $subjects->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
foreach($results AS $row) {
    $encryptedId = $helpers->encryptDecrypt($row['id']);
    $action = '<a class="btn-edit" data-id="'. $row['id'] .'" data-name="'. $row['name'] .'" data-course-no="'. $row['course_no']  .'" data-description="'. $row['description'] .'" data-status="'. $row['is_active'] .'">
                    <i class="fa fa-pencil-square-o"></i>
                </a>';
    $action .= '&nbsp; <a class="btn-delete" data-id="'. $row['id'] .'" data-name="'. $row['name'] .'" data-course-no="'. $row['course_no']  .'"><i class="fa fa-trash-o"></i></a>';
