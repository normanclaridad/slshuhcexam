...
//Get total
$totalRecords = $questions->getTotal($whereCondition, $sortBy);

//Get all tsag
$results = $questions->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
foreach($results AS $row) {
    $encryptedId = $helpers->encryptDecrypt($row['id']);
    $encryptedUrl = $protocol . $_SERVER['HTTP_HOST'] . '/views/questions/edit.php?id=' . $encryptedId;
    $action = '<a class="btn-edit" href="'. $encryptedUrl .'">
                    <i class="fa fa-pencil-square-o"></i>
                </a>';
    $action .= '&nbsp; <a class="btn-delete" data-id="'. $row['id'] .'"><i class="fa fa-trash-o"></i></a>';
    ...
    $questions =  substr(strip_tags($row['question']),0,50) . "...";
